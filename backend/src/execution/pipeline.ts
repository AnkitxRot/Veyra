...
export async function runProject(cfg: AppConfig, projectId: string, workspaceDir: string, spec: RunSpec): Promise<RunResult> {
  const files = await listFiles(workspaceDir);
  const lang = getLang(spec.language ?? '') ?? detectLanguage(files, spec.language ?? null, spec.activeFile ?? null);
  ...
  if (!lang) {
    return { ...base, type: 'no_language', stderr: 'could not determine the project language' };
  }
  ...
  if (lang.compile) {
    try { await fs.access(buildDir, constants.F_OK); await fs.rm(buildDir, { recursive: true, force: true }); } catch {}
  }
