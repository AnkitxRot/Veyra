...
import { listFiles, readProjectFile, writeProjectFile, deleteProjectPath } from '../files/service.js';
...
export async function createSnapshot(
  ...
  const snapshotName = typeof name === 'string' && name.trim() ? name.trim().slice(0, 64) : `Snapshot ${new Date().toLocaleTimeString()}`;
...
export function listSnapshots(db: Db, userId: number, projectId: string): SnapshotRecord[] {
  return db
    .prepare('SELECT * FROM snapshots WHERE project_id = ? AND user_id = ? ORDER BY created_at DESC')
...
export async function restoreSnapshot(
  ...
  const project = requireOwnedProject(db, userId, projectId);
  const row = db
    .prepare('SELECT * FROM snapshots WHERE id = ? AND project_id = ? AND user_id = ?')
  ...
  try {
    ...
    throw new ApiError(404, 'snapshot archive file missing on disk', 'not_found');
  }
...
export async function deleteSnapshot(
  ...
  db.prepare('DELETE FROM snapshots WHERE id = ? AND project_id = ? AND user_id = ?').run(snapshotId, project.id, userId);
}
