...
beforeAll(async () => {
  ...
  const reg = await api.request('POST', '/api/auth/register', { body: { username: 'alice', password: 'secret123' } });
  token = reg.data.token;
  const proj = await api.request('POST', '/api/projects', { token, body: { name: 'demo' } });
...
describe('auth', () => {
  it('rejects duplicate usernames', async () => {
    const r = await api.request('POST', '/api/auth/register', { body: { username: 'alice', password: 'x'.repeat(8) } });
    expect(r.status).toBe(409);
  });

  it('rejects wrong passwords', async () => {
    const r = await api.request('POST', '/api/auth/login', { body: { username: 'alice', password: 'wrong' } });
    expect(r.status).toBe(401);
  });

  it('logs in and reports the current user', async () => {
    const login = await api.request('POST', '/api/auth/login', { body: { username: 'alice', password: 'secret123' } });
    expect(login.status).toBe(200);
    expect(login.data.user.username).toBe('alice');
    const me = await api.request('GET', '/api/auth/me', { token: login.data.token });
...
describe('projects and files', () => {
  ...
  it('creates, lists, and reads back project files', async () => {
    ...
    const got = await api.request('GET', `/api/projects/${projectId}/file?path=main.py`, { token });
    expect(got.status).toBe(200);
    expect(got.data.content).toBe('print("hi")\n');
    const tree = await api.request('GET', `/api/projects/${projectId}/tree`, { token });
    expect(tree.data.tree.some((n: { path: string }) => n.path === 'main.py')).toBe(true);
  });
  ...
  it('moves files', async () => {
    ...
    const got = await api.request('GET', `/api/projects/${projectId}/file?path=src/main.py`, { token });
    expect(got.data.content).toBe('print("hi")\n');
  });

  it('deletes files', async () => {
    await api.request('POST', `/api/projects/${projectId}/delete`, { token, body: { path: 'src/main.py' } });
    const got = await api.request('GET', `/api/projects/${projectId}/file?path=src/main.py`, { token });
...
describe('path traversal protection', () => {
  ...
  for (const p of attempts) {
    ...
    it(`rejects read of "${p}"`, async () => {
      const r = await api.request('GET', `/api/projects/${projectId}/file?path=${encodeURIComponent(p)}`, { token });
      expect(r.status).toBe(400);
    });
  }
  ...
  it.skipIf(IS_WINDOWS)('rejects symlink escapes', async () => {
    const secret = join(tmpdir(), `cloudide-secret-${Math.random().toString(36).slice(2)}.txt`);
    writeFileSync(secret, 'SECRET');
    const ws = join(cfg.workspacesDir, projectId);
    symlinkSync(secret, join(ws, 'link.txt'));
    const r = await api.request('GET', `/api/projects/${projectId}/file?path=link.txt`, { token });
    expect(r.status).toBe(400);
  });

  it.skipIf(IS_WINDOWS)('rejects write through a symlinked directory', async () => {
    const outside = join(tmpdir(), `cloudide-out-${Math.random().toString(36).slice(2)}`);
...
describe.skipIf(!isDockerRunning())('execution via API', () => {
  it('runs a python program through the HTTP API and returns real output', async () => {
    ...
    const r = await api.request('POST', `/api/projects/${projectId}/run`, { token, body: {} });
...
describe('dependency installation command selection', () => {
  ...
  it('case 1: package.json only → npm install', async () => {
    const p = await api.request('POST', '/api/projects', { token, body: { name: 'npm-only' } });
    const pId = p.data.project.id;
    await api.request('POST', `/api/projects/${pId}/file`, {
      token,
      body: { path: 'package.json', content: '{"name":"test","dependencies":{}}' },
    });
...
describe('IDOR & Authorization Security', () => {
  ...
  beforeAll(async () => {
    const reg2 = await api.request('POST', '/api/auth/register', { body: { username: 'bob', password: 'secret123' } });
    user2Token = reg2.data.token;
  });

  it('rejects invalid tokens', async () => {
    const r = await api.request('GET', '/api/projects', { token: 'invalid-token' });
    expect(r.status).toBe(401);
  });

  it('prevents user2 from accessing user1 project files (IDOR)', async () => {
    const r = await api.request('GET', `/api/projects/${projectId}/tree`, { token: user2Token });
    expect(r.status).toBe(404);
  });

  it('prevents user2 from running user1 project', async () => {
    const r = await api.request('POST', `/api/projects/${projectId}/run`, { token: user2Token, body: {} });
    expect(r.status).toBe(404);
  });
});
...
describe.skipIf(!isDockerRunning())('Preview Proxy Security', () => {
  ...
  beforeAll(async () => {
    const reg = await api.request('POST', '/api/auth/register', { body: { username: 'charlie', password: 'secret123' } });
    user2Token = reg.data.token;
  });

  it('rejects unauthenticated requests to proxy', async () => {
    const r = await api.request('GET', `/api/projects/${projectId}/proxy/3000/`);
    expect(r.status).toBe(401);
  });

  it('rejects proxy access via query token', async () => {
    const r = await api.request('GET', `/api/projects/${projectId}/proxy/3000/?token=${token}`);
    expect(r.status).toBe(401);
  });

  it('allows proxy access via bearer token', async () => {
    const r = await api.request('GET', `/api/projects/${projectId}/proxy/3000/`, { token });
    expect(r.status).not.toBe(401);
  });

  it('prevents user2 from proxying user1 project', async () => {
    const r = await api.request('GET', `/api/projects/${projectId}/proxy/3000/`, { token: user2Token });
    expect(r.status).toBe(404);
  });

  it('rejects invalid port formats', async () => {
    const r = await api.request('GET', `/api/projects/${projectId}/proxy/invalid/`, { token });
    expect(r.status).toBe(400);
    expect(r.data.error.message).toBe('Invalid port');
  });

  it('rejects ports below 1024', async () => {
    const r = await api.request('GET', `/api/projects/${projectId}/proxy/80/`, { token });
    expect(r.status).toBe(400);
  });
