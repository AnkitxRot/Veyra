...
import { getSystemCapabilitiesAsync, isDockerRunning, isRunnerImageAvailable } from '../tools.js';
...
export function adminRoutes(cfg: AppConfig, db: Db): Router {
  ...
  const checkLimit = (req: Request, limiter = adminLimiter) => {
    if (!limiter.allow(req.ip ?? 'unknown')) {
      throw new ApiError(429, 'too many admin requests, please slow down', 'rate_limited');
    }
  };
  ...
  router.get('/overview', async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const demoUsersRow = db.prepare("SELECT COUNT(*) as count FROM users WHERE username LIKE 'evaluator_%'").get() as { count: number };
      const totalProjectsRow = db.prepare('SELECT COUNT(*) as count FROM projects').get() as { count: number };
      const totalRunsRow = db.prepare('SELECT COUNT(*) as count FROM runs').get() as { count: number };
      ...
      res.json({
        ...
        aggregateTelemetry: {
          ...
          memoryLimitBytes: cfg.limits.memoryBytes * Math.max(1, activeSandboxes.length),
          pids: aggregatePids,
        },
      });
    } catch (err) {
  ...
  router.get('/telemetry/historical', async (req: Request, res: Response, next: NextFunction) => {
    try {
      checkLimit(req);
  ...
  router.get('/sandboxes', async (req: Request, res: Response, next: NextFunction) => {
    try {
      checkLimit(req);
      const activeSandboxes = await sandboxManager.getAllActiveSandboxes();
      const sandboxes = await Promise.all(
        activeSandboxes.map(async (sb) => {
          ...
          const projectRow = db.prepare(`
            ...
            JOIN users u ON u.id = p.owner_id
            WHERE p.id = ?
          `).get(sb.projectId) as { name: string; created_at: string; username: string } | undefined;
  ...
  router.post('/sandboxes/:containerId/terminate', async (req: Request, res: Response, next: NextFunction) => {
    try {
      checkLimit(req, terminateLimiter);
      const { containerId } = req.params;

      if (!containerId || !/^ide-sandbox-[a-zA-Z0-9_-]+$/.test(containerId)) {
        throw new ApiError(400, 'invalid container id: must be a managed sandbox identifier', 'invalid_container_id');
      }
  ...
  router.get('/executions', (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const languageFilter = typeof req.query.language === 'string' ? req.query.language : undefined;
      const limit = Math.max(1, Math.min(parseInt(String(req.query.limit || '50'), 10), 200));
      ...
      const countRow = db.prepare(`SELECT COUNT(*) as total FROM runs r ${whereSql}`).get(...params) as { total: number };
      ...
      res.json({
        ...
        metrics: {
          ...
          successRatePercent: statsRow?.totalRuns
            ? Math.round(((statsRow.successCount || 0) / statsRow.totalRuns) * 1000) / 10
            : 100,
        },
        runs: rows,
      });
  ...
  router.get('/users/:id', async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (isNaN(userId)) throw new ApiError(400, 'invalid user id', 'invalid_id');

      const userRow = db.prepare(`
        SELECT id, username, role, created_at
        FROM users
        WHERE id = ?
      `).get(userId) as { id: number; username: string; role: string; created_at: string } | undefined;

      if (!userRow) throw new ApiError(404, 'user not found', 'user_not_found');
      ...
      const userSandboxes = activeSandboxes.filter((sb) => userProjectIds.has(sb.projectId));
    ...
    try {
      ...
      if (username !== undefined) {
        if (typeof username !== 'string' || !USERNAME_RE.test(username)) {
          throw new ApiError(400, 'username must be 3-32 alphanumeric characters [a-zA-Z0-9_]', 'invalid_username');
        }
        if (username !== existing.username) {
          const taken = db.prepare('SELECT id FROM users WHERE username = ? AND id != ?').get(username, userId);
          if (taken) throw new ApiError(409, 'username already taken', 'username_taken');
      ...
      if (role !== undefined) {
        if (role !== 'user' && role !== 'admin') {
          throw new ApiError(400, 'role must be user or admin', 'invalid_role');
        }
        ...
        if (existing.role === 'admin' && role === 'user') {
          ...
          if (adminCountRow.count <= 1) {
            throw new ApiError(400, 'Cannot remove admin role: at least one administrator account must exist', 'cannot_demote_last_admin');
          }
        }
        newRole = role;
      }

      db.prepare('UPDATE users SET username = ?, role = ? WHERE id = ?').run(newUsername, newRole, userId);
  ...
  router.post('/users/:id/reset-password', async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const existing = db.prepare('SELECT id, username FROM users WHERE id = ?').get(userId) as { id: number; username: string } | undefined;
      if (!existing) throw new ApiError(404, 'user not found', 'user_not_found');

      const { newPassword } = req.body ?? {};
      if (typeof newPassword !== 'string' || newPassword.length < cfg.minPasswordLength) {
        throw new ApiError(400, `password must be at least ${cfg.minPasswordLength} characters`, 'invalid_password');
      }
  ...
  router.delete('/users/:id', async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (req.user?.id === userId) {
        throw new ApiError(400, 'Cannot delete your own active administrator account', 'cannot_delete_self');
      }

      const target = db.prepare('SELECT id, username, role FROM users WHERE id = ?').get(userId) as
      ...
      if (target.role === 'admin') {
        const adminCountRow = db.prepare("SELECT COUNT(*) as count FROM users WHERE role = 'admin'").get() as { count: number };
        if (adminCountRow.count <= 1) {
          throw new ApiError(400, 'Cannot delete the last remaining administrator account', 'cannot_delete_last_admin');
        }
      }

      // Cascade Cleanup Phase:
      // 1. Projects & Workspaces cleanup
      const projects = db.prepare('SELECT id FROM projects WHERE owner_id = ?').all(userId) as Array<{ id: string }>;
      for (const p of projects) {
        // Stop & reap active Docker container
        try {
          await sandboxManager.stopProjectSandbox(p.id);
        } catch (err) {
          console.error(`[delete-user] failed to stop sandbox for project ${p.id}:`, err);
        }
        ...
        try {
          ...
          console.error(`[delete-user] failed to rm workspace dir for project ${p.id}:`, err);
        }
  ...
  router.get('/audit', (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const { logs, total } = queryAuditLogs(db, { eventType, userId, projectId, limit, offset });
      res.json({ logs, total });
    } catch (err) {
      next(err);
    }
