...
async function measureM12DockerPhases(projectId: string, workspaceDir: string): Promise<PhaseTimings> {
  const containerId = `test-opt12-${projectId}`;
  const netName = `test-net12-${projectId}`;
  ...
  // 3. Net setup (optimized: direct create with catch)
  try {
    await execFileAsync("docker", ["network", "create", netName]);
  } catch {}
  const t3 = performance.now();

  // 4. Docker run (optimized: no pre-rm, no synchronous docker port)
  const hardeningArgs = [
    "--read-only",
  ...
  // 5 & 6. Exec startup and program execution
  let firstByteTime = 0;
...
async function runExecutionBenchmarks() {
  console.log("[M12] Running Execution Benchmarks...");
  const tempDir = join(__dirname, "..", "tmp-m12-opt-" + Date.now());
  mkdirSync(tempDir, { recursive: true });

  try {
    // Single cold start
    const singlePhases = await measureM12DockerPhases(`single-${Date.now()}`, tempDir);

    // Warm execution
    const warmContainer = `test-warm-m12-${Date.now()}`;
    const warmNet = `test-warm-net12-${Date.now()}`;
    ...
    for (const c of [1, 5, 20]) {
      ...
      const tasks = Array.from({ length: c }, async (_, i) => {
        ...
        const phases = await measureM12DockerPhases(p, tempDir);
    ...
    console.log("[M12] Firing 50 simultaneous execution requests via Server...");
    ...
    for (let i = 0; i < 50; i++) {
      const reg = (await fetch(`${server.baseUrl}/api/auth/register`, {
        ...
        body: JSON.stringify({ username: `burst12_u_${i}_${Date.now()}`, password: "load-test-password-1234" }),
      }).then((r) => r.json())) as any;

      const proj = (await fetch(`${server.baseUrl}/api/projects`, {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${reg.token}` },
        body: JSON.stringify({ name: `burst12-p-${i}` }),
      }).then((r) => r.json())) as any;

      await fetch(`${server.baseUrl}/api/projects/${proj.project.id}/file`, {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${reg.token}` },
        body: JSON.stringify({ path: "main.py", content: "print('burst-m12')\n" }),
      });
    ...
    const jsonPath = join(RESULTS_DIR, `m12-exec-optimization-${ts}.json`);
    const mdPath = join(RESULTS_DIR, `m12-exec-optimization-${ts}.md`);

    writeFileSync(jsonPath, JSON.stringify(report, null, 2));

    const md = `# M12 Execution Optimization Benchmark Report
...
|---|---|---|---|
| Docker daemon check (\`docker info\`) | 177.8 ms | ${singlePhases.dockerCheckMs.toFixed(1)} ms | -${(177.8 - singlePhases.dockerCheckMs).toFixed(1)} ms |
...
`;
    ...
    console.log(`[M12] Wrote ${jsonPath} and ${mdPath}`);
...
async function runFilesystemBenchmarks() {
  console.log("[M12] Running Filesystem Benchmarks...");
  const baseTemp = join(__dirname, "..", "tmp-m12-fs-" + Date.now());
  ...
  try {
    ...
    // Concurrent callers (10, 50, 100) on medium project (cached + single-flight)
    ...
    console.log("[M12] Testing /stats telemetry latency...");
    const inactiveStatsTime0 = performance.now();
    await sandboxManager.getContainerStats("inactive-project-sample-12");
    ...
    const ts = new Date().toISOString().replace(/[:.]/g, "-");
    const jsonPath = join(RESULTS_DIR, `m12-fs-optimization-${ts}.json`);
    const mdPath = join(RESULTS_DIR, `m12-fs-optimization-${ts}.md`);

    writeFileSync(jsonPath, JSON.stringify(report, null, 2));

    const md = `# M12 Filesystem Optimization Benchmark Report
...
|---|---|---|---|---|---|
| 10 callers | 14.6 ms | ${concurrencyFsResults[10].p50Ms.toFixed(2)} ms | 14.7 ms | ${concurrencyFsResults[10].p95Ms.toFixed(2)} ms | ${(((14.7 - concurrencyFsResults[10].p95Ms) / 14.7) * 100).toFixed(1)}% |
| 50 callers | 54.1 ms | ${concurrencyFsResults[50].p50Ms.toFixed(2)} ms | 54.2 ms | ${concurrencyFsResults[50].p95Ms.toFixed(2)} ms | ${(((54.2 - concurrencyFsResults[50].p95Ms) / 54.2) * 100).toFixed(1)}% |
| 100 callers | 109.5 ms | ${concurrencyFsResults[100].p50Ms.toFixed(2)} ms | 109.6 ms | ${concurrencyFsResults[100].p95Ms.toFixed(2)} ms | ${(((109.6 - concurrencyFsResults[100].p95Ms) / 109.6) * 100).toFixed(1)}% |

## Inactive /stats Telemetry Latency

- **M10 Baseline (failed \`docker stats\` execution)**: ~50 ms
- **M12 Optimized (in-memory fast path)**: **${inactiveStatsTime.toFixed(2)} ms** (Speedup: ~${(50 / Math.max(0.01, inactiveStatsTime)).toFixed(1)}x)
`;
...
async function main() {
  ...
  console.log("[M12] All optimization benchmarks complete.");
}
