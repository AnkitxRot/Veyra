...
export function authRoutes(db: Db, cfg: AppConfig): Router {
  ...
  router.post('/register', async (req, res, next) => {
    try {
      ...
      res.status(201).json({ token, user: { id: userId, username, role: 'user' } });
    } catch (err) {
      next(err);
    }
  });
    ...
    try {
      ...
      if (row.role !== 'admin') {
        recordAuditLog(db, {
          userId: row.id,
          eventType: 'AUTH_FAILED_LOGIN',
          details: { username: row.username, reason: 'insufficient_privileges_non_admin' },
          ipAddress: req.ip,
        });
      ...
      res.json({ token, user: { id: row.id, username: row.username, role: 'admin' } });
    } catch (err) {
      next(err);
    }
  });
  ...
  router.post('/demo', async (req, res, next) => {
    try {
      ...
      const info = db.prepare('INSERT INTO users (username, password_hash, role) VALUES (?, ?, ?)').run(guestUsername, hash, 'user');
      const userId = Number(info.lastInsertRowid);
      ...
      res.status(201).json({
        token,
        user: { id: userId, username: guestUsername, role: 'user', isDemo: true },
        project,
      });
    } catch (err) {
      next(err);
